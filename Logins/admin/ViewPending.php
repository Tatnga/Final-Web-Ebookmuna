...
$db = new firebaseRDB($databaseURL);
  ...
  <select class="custom-select" id="inputGroupSelect02" onchange="searchTable()">
    ...
    <option value="4">4</option>
			...
			<h1 style="margin-top:75px;text-align:center;color:#7b200a;font-size:80px; font-weight:bold;text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black" class="text-center">View Pending Registration</h1>
  ...
  <table class="table table-striped" width="100%" align="center">
    <thead>
      <tr bgcolor="#dddddd">
        ...
        <th onclick="sortTable(8)">Penalty</th>
        <th onclick="sortTable(9)">Usertype</th>
        <th onclick="sortTable(10)">Status</th>
        <th onclick="sortTable(11)">Date</th>
...
if (is_array($data)) {
  foreach ($data as $id => $users) {
    if (is_array($users) && array_key_exists('status', $users) && $users['status'] == 'pending') {
      $registration_date = date('F j, Y', strtotime($users['registration_date']));
      if (!empty($users['name']) && !empty($users['useridno']) && !empty($users['contact']) && !empty($users['email']) && !empty($users['address']) && !empty($users['useryear']) && !empty($users['password']) && !empty($users['usercourse']) && !empty($users['penalty']) && !empty($users['user_type']) && !empty($users['status']) && !empty($registration_date)) {
        echo "<tr>
                <td>{$users['name']}</td>
