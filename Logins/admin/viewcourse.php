...
<div class="input-group mb-3">
  ...
  <select class="custom-select" id="inputGroupSelect02" onchange="searchTable()">
	...
	<h1 style="margin-top:75px;text-align:center;color:#7b200a;font-size:80px; font-weight:bold;text-shadow: -1px 0 black, 0 1px black, 1px 0 black, 0 -1px black" class="text-center">View Course</h1>
	<div class="user">
        ...
        if (is_array($data)) {
          foreach ($data as $id => $users) {
            if (is_array($users) && isset($users['childno'])) {
              echo "<tr>
                      <td>{$users['childno']}</td>
                      <td>{$users['name1']}</td>
                      <td>{$users['name2']}</td>
                    ...
                    echo "</div>
                      ...
                      <td><a href='editcourse.php?id=${id}'><i class='fa fa-pencil'></i></a></td>
                      <td><a href='deletecourse.php?id=$id'><i class='fa fa-trash'></i></a></td>
                    </tr>";
            }
          }
        }
