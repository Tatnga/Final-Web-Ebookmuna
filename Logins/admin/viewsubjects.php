...
<div class="input-group mb-3">
  ...
  <select class="custom-select" id="inputGroupSelect02" onchange="searchTable()">
	...
	<div class="user">
        <a href="addsubjects.php" class="btn">Add New Subjects</a>
          &nbsp;  &nbsp; &nbsp; &nbsp;
  </div>
...
if (is_array($data)) {
  foreach ($data as $id => $users) {
      if (is_array($users) && isset($users['name1']) ) {
          echo "<tr>
          <td>{$users['childno']}</td>
              <td>{$users['name1']}</td>
              <td><a href='editsubjects.php?id=${id}'><i class='fa fa-pencil'></i></a></td>
              <td><a href='deletesubject.php?id=$id'><i class='fa fa-trash'></i></a></td>
          </tr>";
      }
  }
}
